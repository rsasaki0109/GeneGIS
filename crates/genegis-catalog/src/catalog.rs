...
/// Well-known dataset id for the synthetic transit corridor fixture (OD-3).
pub const NAGOYA_TRANSIT_ID: &str = "nagoya-transit";

/// Well-known dataset id for the synthetic 500m population-mesh fixture.
pub const NAGOYA_POPULATION_MESH_ID: &str = "nagoya-population-mesh";
...
/// Path to the bundled transit corridor fixture.
///
/// Set `GENEGIS_TRANSIT_PATH` (and `GENEGIS_TRANSIT_SHA`) to run the multimodal
/// accessibility workflow on the real 国土数値情報 N02/N07 corridors produced by
/// `scripts/fetch-mlit-transit.py`. Checksums must be re-declared alongside the
/// path so receipts stay fail-closed.
pub fn nagoya_transit_path() -> &'static str {
    if let Ok(path) = std::env::var("GENEGIS_TRANSIT_PATH") {
        return Box::leak(path.into_boxed_str());
    }
    concat!(
        env!("CARGO_MANIFEST_DIR"),
...
/// Path to the 3D scene COPC point cloud (Phase 14 M0/M1 fixture).
///
/// Set `GENEGIS_COPC_PATH` to run the 3D scene on a real point cloud (e.g. a
/// GSI airborne-LiDAR-derived COPC); the digest must be re-declared through the
/// acceptance request so receipts stay fail-closed.
pub fn nagoya_scene_copc_path() -> &'static str {
    if let Ok(path) = std::env::var("GENEGIS_COPC_PATH") {
        return Box::leak(path.into_boxed_str());
    }
    concat!(
        env!("CARGO_MANIFEST_DIR"),
...
/// Path to the 3D scene LOD1 buildings fixture.
///
/// Set `GENEGIS_BUILDINGS_PATH` to run the 3D scene on real PLATEAU-derived
/// LOD1 buildings; the digest must be re-declared through the acceptance
/// request so receipts stay fail-closed.
pub fn nagoya_scene_lod1_path() -> &'static str {
    if let Ok(path) = std::env::var("GENEGIS_BUILDINGS_PATH") {
        return Box::leak(path.into_boxed_str());
    }
    concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../examples/nagoya-population-density/data/nagoya-scene-lod1.json"
    )
}
...
/// Path to the bundled 500m population-mesh fixture.
///
/// Set `GENEGIS_POPULATION_MESH_PATH` (and `GENEGIS_POPULATION_MESH_SHA`) to run
/// the same density workflow on the licensed real e-Stat 500m mesh produced by
/// `scripts/fetch-estat-mesh.py`. Checksums must be re-declared alongside the
/// path so receipts stay fail-closed.
pub fn nagoya_population_mesh_path() -> &'static str {
...
pub fn alpha_catalog() -> Catalog {
    ...
    catalog.register(nagoya_population_mesh_record());
    catalog.register(nagoya_transit_record());
...
fn nagoya_population_mesh_record() -> DatasetRecord {
    DatasetRecord {
        id: NAGOYA_POPULATION_MESH_ID.into(),
        title: "Nagoya synthetic 500m population mesh".into(),
        description: "Deterministic 500m population-mesh fixture that conserves the official 2020 ward totals by area-weighted cell allocation. Generated by scripts/build-nagoya-population-mesh.py; NOT real census mesh data. Set GENEGIS_POPULATION_MESH_PATH for the licensed e-Stat mesh.".into(),
        format: DatasetFormat::geojson(),
        crs: "EPSG:4326".into(),
        bbox: BoundingBox::new(136.78, 35.02, 137.08, 35.28),
        uri: nagoya_population_mesh_path().into(),
        license: "CC0-1.0 (synthetic fixture; 参考: e-Stat 統計地理情報システム 地域メッシュ統計)".into(),
        checksum: Some(declared_checksum(
            "GENEGIS_POPULATION_MESH_SHA",
            "2acb7ec476599aad5ad7b6ce5dbccfc1b5bca9f355440c66fec3f9238ea34f92",
        )),
        source_version: Some("nagoya-population-mesh-fixture-v1".into()),
        tags: vec!["nagoya".into(), "population".into(), "mesh".into(), "demo".into()],
    }
}

fn nagoya_transit_record() -> DatasetRecord {
    DatasetRecord {
        id: NAGOYA_TRANSIT_ID.into(),
        title: "Nagoya synthetic transit corridors (rail/bus)".into(),
        description: "Deterministic rail corridors over the Nagoya walk grid (名古屋駅 area → 金山/千種/大曽根) with per-stop lines and ride times. Generated by scripts/build-nagoya-transit.py; NOT real MLIT N02/N07 data. Set GENEGIS_TRANSIT_PATH for the licensed real corridors.".into(),
        format: DatasetFormat::geojson(),
        crs: "EPSG:4326".into(),
        bbox: BoundingBox::new(136.87, 35.11, 136.95, 35.20),
        uri: nagoya_transit_path().into(),

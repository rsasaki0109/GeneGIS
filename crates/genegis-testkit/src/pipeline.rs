...
pub fn benchmark_pipeline(warmup: u32, iterations: u32) -> Result<BenchmarkSample, TestkitError> {
    let durations = time_iterations(warmup, iterations, || {
        ...
        if !result.duckdb_verified {
            return Err(TestkitError::Pipeline("DuckDB verification failed".into()));
        }
        Ok(())
    })?;
